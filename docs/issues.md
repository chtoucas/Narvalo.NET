...
- Update CodeFormatter when it supports C# 7.0 and see if it still has a
  problem w/ our code analysis ruleset. We also need to check that we can exclude
  some files from copyright header checking. After that, change from `format-code.cmd`
  to `format-code.ps1`.
