namespace Narvalo
{
    ...
    {
        private static ImageCodecInfo AvailableJpegEncoder;

        public static ImageCodecInfo JpegEncoder
        {
            get
            {
                if (AvailableJpegEncoder == null) {
                    ...
                    foreach (ImageCodecInfo info in ici) {
                        if (info.FormatID == ImageFormat.Jpeg.Guid) {
                            AvailableJpegEncoder = info;
                            break;
                        }
                    }
                }

                return AvailableJpegEncoder;
            }
        }
        ...
        private static void SaveAsJpeg(Bitmap bitmap, Stream outStream, long level)
        {
            using (var ep = new EncoderParameters(1)) {
                ...
                bitmap.Save(outStream, AvailableJpegEncoder, ep);
            }
        }
    }
}
